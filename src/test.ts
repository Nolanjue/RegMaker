import { insertRegValues } from "./utils";
import {regexDict, userParams,SpecialCharacter, specialCharacterDict,  DefaultValues, userTypes, RequiredFlags } from './types'
...
class RegExMaker {
    ...
    * 
    */
    public create_exp(desired_example: string, value_attr: regexDict, capture_values: boolean = false, flags?: string[], newTypes?: userParams): string {
        let inserted_EXP = desired_example;
        let final_EXP = '';
        let filled_indices: { [key: number]: [number, number, string] } = {};

        try {
            for (let regex_character of Object.keys(value_attr)) {
                let desired_value = this.regexMap[regex_character as keyof regexDict];
                const current_value = value_attr[regex_character as keyof DefaultValues];
            ...
            if (newTypes) {
                for (let regex_character of Object.keys(newTypes)) {
                    ...
                    let desired_value = this.usersMap[regex_character as keyof userTypes];
            ...
            while (orig_str_index < inserted_EXP.length) {
                if (!filled_indices[orig_str_index]) {
                    ...
                    final_EXP += specialCharacterDict[char as SpecialCharacter] ? '\\' + char : char;
                    orig_str_index++;
                } else {
                    const [start, end, regexKey] = filled_indices[orig_str_index];
                    const pattern = this.regexMap[regexKey as keyof regexDict] || this.usersMap[regexKey as keyof regexDict];
            ...
            this.current_regex = new RegExp(final_EXP, flags?.join('') || '');
            return final_EXP;
        } catch (error) {
    ...
    **/
    public add_custom(key: string, value: string) {
    ...
    */
     ...
     */
    public remove_custom(key: string) {
    ...
    * ```
     ...
     */
    public merge_exp(
        desired_example: string,
        value_attr: regexDict,
        capture_values: boolean = false,
        flags?: RequiredFlags,
        newTypes?: userParams
    ) {
        if(!desired_example){
        ...
        if (this.current_regex) {
            ...
            this.current_regex = new RegExp(`${old_regex}|${new_regex}`, flags?.join('') || '');
        } else {
            this.create_exp(desired_example, value_attr, capture_values, flags, newTypes);
        }
     ...
     */
    public test(stringVal: string): boolean {
    ...
    */
    public match(stringVal: string): string[] {
     ...
     * @returns {string[]} - The matched values.
     */
    public exec(stringVal: string): string[] {
 ...
 * @returns {string[]} - The matched values
 */
    public matchAll(stringVal: string, fetchCaptures: boolean): string[] {
        ...
        return matches
            .map(m => m.filter(val => val !== undefined)) // Filter undefined values from full match and capture groups
            .flat();
    }
    /**
 ...
 */
    public replace(stringVal: string, groups: string, desired_regex?: RegExp) {
        //allows to return 
 ...
 */
export function RegMake(): RegExMaker {
